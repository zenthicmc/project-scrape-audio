...
export default function LoginPage() {
  ...
  const [errorMsg, setErrorMsg] = useState(
    error === "CredentialsSignin" ? "Email atau password salah." : error ? "Terjadi kesalahan. Coba lagi." : ""
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      setErrorMsg(result.error === "CredentialsSignin" ? "Email atau password salah." : result.error);
      setLoading(false);
    } else {
  ...
  return (
    <div className="w-full max-w-md">
      <div className="bg-card border border-border rounded-2xl p-8">
        ...
        <p className="text-muted-foreground text-sm mb-8">Masuk ke akun ScriptAI Anda</p>
